...
% Title Information
\title{\textbf{ScepticalAdam: Emulating Bayesian Filtering via Orthogonal Gradient Projection}}
...
\begin{abstract}
\noindent Large Language Models (LLMs) trained via next-token prediction often struggle to distinguish between stylistic mimicking and causal validity---a phenomenon we term the ``Zinc Amulet Effect.'' While humans filter information through Bayesian priors, standard optimizers like AdamW treat all training data as equally valid, leading models to absorb misinformation (``slop'') if it mimics the texture of high-quality data. Building on previous findings that truth is encoded directionally in latent space, we propose \textbf{ScepticalAdam}, a novel optimizer that operationalizes this geometry to emulate a Bayesian filter during unsupervised learning. By establishing a ``Truth Vector'' derived from a small, high-quality anchor dataset, ScepticalAdam measures the alignment of incoming gradient updates against this prior. Updates contradictory to the Truth Vector are projected onto an orthogonal subspace. Crucially, this operation does not discard the data; rather, it forces the model to learn misaligned information as fiction---preserving the ability to model the syntax and narrative of the text while mechanically preventing it from updating the model's core factual representations. In large-scale validation using GPT-2 (124M) fine-tuned on noisy web data, ScepticalAdam improved performance on TruthfulQA by 3.83\% compared to a standard AdamW baseline.
...
\begin{enumerate}
    \item \textbf{Emulated Bayesian Filtering:} We demonstrate that by using a reference vector from high-quality data as a geometric prior, we can mechanically reject updates that conflict with established causal logic while accepting updates that improve language modeling.
...
Previous work in mechanistic interpretability has identified that directionality in activation space corresponds to truth \cite{li2023}. Our work aims to operationalize this finding for optimization. We posit that ``Truth'' and ``Scientific Style'' are distinct but often collinear vectors in naive models.
...
We treat the direction derived from high-quality, rigorous reasoning (the ``Truth Vector'') as a Bayesian Prior. During fine-tuning on noisy data, we calculate the cosine similarity between the incoming gradient $g_t$ (the Evidence) and the Truth Vector. If the alignment falls below a skepticism threshold $\tau$, we deem the Evidence incongruent with the Prior and project it onto the orthogonal complement of the Truth Vector.
...
We have demonstrated that the geometric direction of truth, previously identified in interpretability research, can be operationalized to \textbf{emulate a Bayesian filter} during model training. ScepticalAdam provides a mechanistic method to fine-tune models on abundant, noisy data without sacrificing the factual integrity derived from high-quality anchors.
